...
class TestLogin:
    ...
    def test_agregar_items_carrito(self):
        ...
        time.sleep(1)
        esperado_mensaje = "Carnes"
        actual_categoria = self.driver.find_element(By.XPATH, "//h2[@id='titleCategoria4']").text
        ...
        assert esperado_mensaje == actual_categoria, f"LOS VALORES NO COINCIDEN: Actual: {actual_categoria}, Esperado: {esperado_mensaje}"
    ...
    def test_eliminar_item_carrito(self):
        ...
        self.driver.find_element(By.XPATH, "//a[@href='vercarrito' and contains(@class, 'btn')]").click()
        time.sleep(1)
        self.driver.find_element(By.XPATH, "//a[@id='eliminarItem110']").click()
        time.sleep(2)
        contador_esperado = 2
        contador_actual = len(self.driver.find_elements(By.XPATH, "//tbody//tr"))
        assert contador_esperado == contador_actual, f"LOS VALORES NO COINCIDEN: Actual: {contador_actual}, Esperado: {contador_esperado}"
    ...
    def test_producto_no_disponible(self):
        ...
        self.driver.find_element(By.XPATH, "//a[text()='Realiza tu PEDIDO']").click()
        time.sleep(1)
        self.driver.execute_script("window.scrollTo(800,500)")
        time.sleep(1)
        esperado_mensaje = "Producto no disponible"
        actual_mensaje = self.driver.find_element(By.XPATH, "//div[@id='divProducto142']//child::p").text
        assert esperado_mensaje in actual_mensaje, f"LOS VALORES NO COINCIDEN: Actual: {actual_mensaje}, Esperado: {esperado_mensaje}"
